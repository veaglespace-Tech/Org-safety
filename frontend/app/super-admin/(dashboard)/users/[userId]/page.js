...
export default function SuperAdminUserDetailStandalonePage() {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetSuperAdminUserByIdQuery(userId, {
      ...
      <div className="light-glow-card-static space-y-5 rounded-[1.9rem] p-6">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">
            User ID: {`ATTY-${toDisplayText(organization?.organizationCode, "GLOBAL")}-${user.id}`}
          </p>
        </div>
